...
export default function RemoteTrackerView() {
  ...
  const hasAnyRemoteEntries = useMemo(
    () => (calendarQuery.data ?? []).some((d) => d.hasActivity),
    [calendarQuery.data],
  );

  async function runTracker(action: TrackerAction) {
  ...
  function onLogSubmit(
    ...
    void (async () => {
      try {
        ...
        setStatus(err instanceof Error ? err.message : "Could not add entry.", true);
      }
    })();
  }
  ...
  return (
    <>
      <TrackerDialog
        ...
        description="Basal metabolic rate used for calendar colors (net intake vs this value). Synced with your account."
        primaryLabel="Save"
