...
export const GET: APIRoute = async ({ locals, url }) => {
  ...
  const rows = await listMealsForDate(session.user.id, date);
  return new Response(JSON.stringify({ meals: rows.map(mealRowToWire) }), {
    headers: { "Content-Type": "application/json" },
  });
};
...
export const POST: APIRoute = async ({ locals, request }) => {
  ...
  const displayOrder = await nextMealDisplayOrder(session.user.id, date);
  ...
  return new Response(JSON.stringify({ meal: mealRowToWire(row) }), {
    status: 201,
    headers: { "Content-Type": "application/json" },
  });
};
